...
const char *MODEL = "esp8266-mqtt"; // do not change if you want OTA updates
const char *DEVICENAME = "table-leds";
const char *DEVCHANNEL = "/table-leds";
...
#define NUM_LEDS_ONE 300 // Number of LED's

// #define LED_DT_TWO D12 // Serial data pin
#define LED_DT_TWO D2    // Serial data pin
#define NUM_LEDS_TWO 300 // Number of LED's
...
void setup()
{
  ...
  setupWithCredentials();
}

// SETUP
// TODO: make separate connect func
void setupWithCredentials()
{
  if (SPIFFS.exists("/credentials.txt"))
  {
    ...
    if (!f)
    {
      Serial.println("Credentials file failed to open! Make sure you've changed and renamed data/credentials-example.txt to data/credentials.txt");
    }
    else
    {
      String wifiSettings = fileRead("/credentials.txt");
      ...
      if (root.success())
      {
        ...
        const char *login = root["mqtt-username"];
        const char *mqttPass = root["mqtt-password"];
        ...
        Serial.print("\nconnecting...");
        while (!client.connect(DEVICENAME, login, mqttPass)) {
          Serial.print(".");
          delay(1000);
        }
        ...
        //        client.subscribe(DEVCHANNEL);
      }
      else
      {
        Serial.println("Could not parse credentials.txt as JSON");
      }
    }
  }
  else
  {
    Serial.println("credentials.json file not found!\nPlease change wifi_example.txt and rename it to credentials.txt");
  }
}
...
void parseMessage(String message)
{
  ...
  if (root.success())
  {
    ...
    else if (strcmp(action, "send-hello") == 0)
    {
      sendHello();
    }
    // reboots the device for some reason. disable for now as non-essential
